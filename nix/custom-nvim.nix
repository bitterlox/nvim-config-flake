{ pkgs }:
    let
      ...
      matchConfigFiles = configAttrSet: warnMsg: builtins.filter (e: e != null) (
        builtins.map ( file-name: let
          file-path = (builtins.getAttr file-name configAttrSet);
          in if (builtins.elem file-name plugin-names) then file-path
             else pkgs.lib.trivial.warn "${warnMsg} ${file-path}" null
        ) (builtins.attrNames configAttrSet)
      );

      matched-config = matchConfigFiles plugin-config "unused plugin config";
      matched-keybindings = matchConfigFiles plugin-keybindings "unused plugin keybinding";
